...
def featureMatching(image_ref, image_cur, matching_algorithm, threshold_value, output_path):
    ...
    key_point1, descriptor1 = detector.detectAndCompute(image_ref,None) #create SIFT descriptor
    key_point2, descriptor2 = detector.detectAndCompute(image_cur,None)
    sift = cv2.SIFT_create()
    
    #LAB 4: put your feature matching code after this line
    matches = []
    if(matching_algorithm == 1): #match by distance
        key_point1, descriptor1 = detector.detectAndCompute(image_ref,None) #create SIFT descriptor
        key_point2, descriptor2 = detector.detectAndCompute(image_cur,None)
        bf = cv2.BFMatcher(cv2.NORM_L2, True)
        matches_dis = bf.match(descriptor1, descriptor2)
        matches_dis = sorted(matches_dis, key= lambda x: x.distance)
        for i in matches_dis:
            if (i.distance < threshold_value):
                matches.append(i)
    else:
        key_point1, descriptor1 = sift.detectAndCompute(image_ref,None) 
        key_point2, descriptor2 = sift.detectAndCompute(image_cur,None)
        bf = cv2.BFMatcher()
        if(matching_algorithm == 2):
            matches_dis = bf.knnMatch(descriptor1, descriptor2, 1)
            for m in matches_dis:
                if m[0].distance < threshold_value:
                    matches.append([m[0]])
        elif(matching_algorithm == 3):
            match = bf.knnMatch(descriptor1, descriptor2, 2) #nndr
            #apply ratio test
            matches = []
            for m,n in match:
    ...
    if(matching_algorithm == 1 ):
        img3 = cv2.drawMatches(image_ref, key_point1, image_cur, key_point2, matches[:150],None, flags=2)
    elif(matching_algorithm==3 or matching_algorithm == 2):

        img3 = cv2.drawMatchesKnn(image_ref, key_point1, image_cur, key_point2, matches[:150], None, flags=2)
    fig = plt.figure()
...
def printMatchesToFile(matches, output_path):
    ...
    for i in range(len(matches)-1):
        ...
        if(type(matches[i]) is list):
            for j in matches[i]:
                f = round(j.distance, 2)
                output_file.write(str(f) + ' ')
        else:
            f = round(matches[i].distance, 2)
            output_file.write(str(f) + ' ')
        output_file.write('\n')            
    output_file.close()
